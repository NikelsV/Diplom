...
- скачивается официальный образ `postgres:16-alpine`,
- запускаются два контейнера: `webmap-db` и `webmap-web`,
...
Должны быть видны два контейнера со статусом `Up`.
...
Чтобы зайти в админку, нужен учётный пользователь Django:
...
- http://localhost:8000/ — главная карта России.
